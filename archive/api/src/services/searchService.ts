...
class SearchService {
   ...
   */
  public async search(query: string | any, options: Partial<SearchOptions> = {}): Promise<SearchResult[]> {
    ...
    if (typeof query === 'string') {
      ...
      try {
        ...
        logger.info(`Received object query: ${JSON.stringify(query, null, 2)}`);
        
        // If the query is an object with a query property, extract it
        if (query.query && typeof query.query === 'string') {
          cleanedQuery = query.query.trim();
          
          // Also extract any other options from the query object
          if (query.maxResults && typeof query.maxResults === 'number') {
            searchOptions.maxResults = query.maxResults;
          }
          if (query.region && typeof query.region === 'string') {
            searchOptions.region = query.region;
          }
          if (query.safeSearch !== undefined) {
            searchOptions.safeSearch = !!query.safeSearch;
          }
          if (query.filters) {
            searchOptions.filters = query.filters;
          }
        } else {
          // If it doesn't have a query property, convert the whole object to a string
          cleanedQuery = String(query);
          // Make sure it's not just [object Object]
          if (cleanedQuery === '[object Object]') {
            const errorMsg = `Invalid search query object: missing 'query' property`;
            logger.error(errorMsg, { receivedObject: query });
            throw new Error(errorMsg);
          }
        }
      } catch (error:any) {
        logger.error(`Error processing query object: ${error}`, { query });
        throw new Error(`Invalid search query: ${error.message}`);
      }
    } else {
      // Handle other types by converting to string
      cleanedQuery = String(query).trim();
      
      // Log for debugging
      logger.info(`Converted non-string query to string: "${cleanedQuery}" (type: ${typeof query})`);
    }
    
    // Final validation
    if (!cleanedQuery || cleanedQuery.length === 0) {
      const errorMsg = `Empty search query after processing`;
      logger.error(errorMsg);
      throw new Error(errorMsg);
    }
    
    // Log both the query and options for debugging
    logger.info(`Performing search with query: "${cleanedQuery}"`, { 
      originalQuery: query,
      processedQuery: cleanedQuery, 
      options: JSON.stringify(searchOptions, null, 2)
    });
    
    try {
      // Currently only supporting DuckDuckGo
      // In the future, this could be extended to support other search engines
      const results = await this.duckDuckGoSearch(cleanedQuery, searchOptions);
      
      // Log search completion with result count
      logger.info(`Search completed for query: "${cleanedQuery}" - found ${results.length} results`);
      
      return results;
    } catch (error) {
      logger.error(`Search failed for query: "${cleanedQuery}"`, { error, originalQuery: query });
      throw error;
    }
  }

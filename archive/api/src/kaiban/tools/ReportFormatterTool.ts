  ...
  private formatAsJSON(
    ...
    if (sources.length > 0) {
      report.sources = sources.map(source => {
        const formattedSource: any = {
          url: source.url
        };
        
        if (source.title) formattedSource.title = source.title;
        if (source.author) formattedSource.author = source.author;
        if (source.publishDate) formattedSource.publishDate = source.publishDate;

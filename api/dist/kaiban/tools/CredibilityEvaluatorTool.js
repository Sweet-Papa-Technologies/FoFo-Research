 ...
 */
class CredibilityEvaluatorTool extends tools_1.DynamicTool {
    constructor() {
        super({
            ...
            func: async (input) => {
                try {
                    ...
                    if (!input || input === "undefined") {
                        logger_1.logger.error("CredibilityEvaluatorTool received undefined or empty input");
                    ...
                    const overallScore = this.calculateCredibilityScore(credibilityFactors, contentAnalysis, metadataAnalysis);
                    ...
                    logger_1.logger.error("Error in CredibilityEvaluatorTool:", error);
                    return JSON.stringify({
    ...
    analyzeDomainCredibility(url) {
        ...
        for (const knownDomain in knownSources) {
            if (domain.includes(knownDomain)) {
                sourceInfo = knownSources[knownDomain];
                break;
            }
        }
    ...
    assessTldReputation(domain) {
        const tld = domain.split('.').pop();
        // Basic TLD reputation assessment
        const tldReputations = {
            "edu": "high",
            "gov": "high",
            "org": "medium",
            "com": "medium",
        ...
        return tldReputations[tld] || "unknown";
    }
    ...
    calculateCredibilityScore(domainFactors, contentFactors, metadataFactors) {
        // This is a simplified scoring algorithm
        // In production, would use a weighted model
        let score = 50; // Start at neutral
        // Domain factors
        if (domainFactors.sourceInfo.credibility === "very high")
            score += 25;
        else if (domainFactors.sourceInfo.credibility === "high")
            score += 15;
        else if (domainFactors.sourceInfo.credibility === "medium")
            score += 5;
        else if (domainFactors.sourceInfo.credibility === "low")
            score -= 10;
        if (domainFactors.domainFactors.secureConnection)
            score += 5;
        if (domainFactors.domainFactors.tldReputation === "high")
            score += 10;
        else if (domainFactors.domainFactors.tldReputation === "medium")
            score += 5;
        else if (domainFactors.domainFactors.tldReputation === "low")
            score -= 5;
        // Content factors (if available)
        if (Object.keys(contentFactors).length > 0) {
            if (contentFactors.hasReferences)
                score += 10;
            if (contentFactors.contentLength > 2000)
                score += 5;
            if (contentFactors.readabilityScore === "high")
                score += 5;
        }
        // Metadata factors (if available)
        if (Object.keys(metadataFactors).length > 0) {
            if (metadataFactors.hasAuthor)
                score += 5;
            if (metadataFactors.hasPublishDate)
                score += 5;
            if (metadataFactors.isRecent)
                score += 5;
        }
...
exports.CredibilityEvaluatorTool = CredibilityEvaluatorTool;

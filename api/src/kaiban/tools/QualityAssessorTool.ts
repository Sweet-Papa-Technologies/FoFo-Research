...
export class QualityAssessorTool extends Tool {
  ...
  private assessSourceDiversity(sources: Array<{ url: string; sourceType?: string }>) {
    ...
    if (diversityRatio >= 0.9) diversityScore += 40;
    else if (diversityRatio >= 0.7) diversityScore += 30;
    else if (diversityRatio >= 0.5) diversityScore += 20;
    else if (diversityRatio >= 0.3) diversityScore += 10;
    
    // Source type diversity (0-40 points)
    if (uniqueSourceTypes.size >= 5) diversityScore += 40;
    else if (uniqueSourceTypes.size >= 4) diversityScore += 30;
    else if (uniqueSourceTypes.size >= 3) diversityScore += 20;
    else if (uniqueSourceTypes.size >= 2) diversityScore += 10;
    
    // Source count (0-20 points)
    if (sources.length >= 10) diversityScore += 20;
    else if (sources.length >= 7) diversityScore += 15;
    else if (sources.length >= 5) diversityScore += 10;
    else if (sources.length >= 3) diversityScore += 5;
  ...
  private assessSourceQuality(sources: Array<{ credibilityScore?: number }>) {
    ...
    if (averageCredibility >= 80) qualityScore += 50;
    else if (averageCredibility >= 70) qualityScore += 40;
    else if (averageCredibility >= 60) qualityScore += 30;
    else if (averageCredibility >= 50) qualityScore += 20;
    else if (averageCredibility >= 40) qualityScore += 10;
    
    // High-quality sources (0-30 points)
    const highQualityRatio = highQualitySourceCount / sources.length;
    if (highQualityRatio >= 0.7) qualityScore += 30;
    else if (highQualityRatio >= 0.5) qualityScore += 20;
    else if (highQualityRatio >= 0.3) qualityScore += 10;
    
    // Penalty for low-quality sources (-20 to 0 points)
    const lowQualityRatio = lowQualitySourceCount / sources.length;
    if (lowQualityRatio >= 0.5) qualityScore -= 20;
    else if (lowQualityRatio >= 0.3) qualityScore -= 15;
    else if (lowQualityRatio >= 0.2) qualityScore -= 10;
    else if (lowQualityRatio >= 0.1) qualityScore -= 5;
    
    // Coverage of sources with credibility data (0-20 points)
    const credibilityCoverage = sourcesWithScores.length / sources.length;
    if (credibilityCoverage >= 0.9) qualityScore += 20;
    else if (credibilityCoverage >= 0.7) qualityScore += 15;
    else if (credibilityCoverage >= 0.5) qualityScore += 10;
    else if (credibilityCoverage >= 0.3) qualityScore += 5;
  ...
  private assessCompleteness(keyFindings: string[], topic: string, researchGoal?: string) {
    ...
    if (keyFindings.length >= 10) completenessScore += 50;
    else if (keyFindings.length >= 8) completenessScore += 40;
    else if (keyFindings.length >= 6) completenessScore += 30;
    else if (keyFindings.length >= 4) completenessScore += 20;
    else if (keyFindings.length >= 2) completenessScore += 10;
    ...
    if (avgFindingLength >= 150) completenessScore += 25;
    else if (avgFindingLength >= 100) completenessScore += 20;
    else if (avgFindingLength >= 75) completenessScore += 15;
    else if (avgFindingLength >= 50) completenessScore += 10;
    else if (avgFindingLength >= 25) completenessScore += 5;
  ...
  private assessResearchDepth(
    ...
    if (sources.length >= 12) depthScore += 50;
    else if (sources.length >= 10) depthScore += 40;
    else if (sources.length >= 8) depthScore += 30;
    else if (sources.length >= 6) depthScore += 20;
    else if (sources.length >= 4) depthScore += 10;
    
    // Finding depth - ratio of findings to sources (0-50 points)
    const findingToSourceRatio = keyFindings.length / sources.length;
    if (findingToSourceRatio >= 2.0) depthScore += 50;
    else if (findingToSourceRatio >= 1.5) depthScore += 40;
    else if (findingToSourceRatio >= 1.0) depthScore += 30;
    else if (findingToSourceRatio >= 0.7) depthScore += 20;
    else if (findingToSourceRatio >= 0.5) depthScore += 10;
  ...
  private calculateOverallScore(...scores: number[]) {
    // Average the component scores
    const validScores = scores.filter(score => !isNaN(score));
    if (validScores.length === 0) return 50; // Default to neutral
    
    const sum = validScores.reduce((total, score) => total + score, 0);
    return Math.round(sum / validScores.length);
  }
  
  private getQualityRating(score: number) {
  ...
  private identifyImprovementAreas(
    sourceDiversity: { score: number },
    sourceQuality: { score: number },
    completeness: { score: number },
    researchDepth: { score: number }
  ) {

...
import { SummaryAgent, ResearchDirectorAgent } from '../agents';
 ...
 */
export class SynthesisTeam {
  private team: Team;
  private summaryAgent: SummaryAgent;
  private directorAgent: ResearchDirectorAgent;
  ...
  constructor(config?: {
    ...
    try {
      ...
      this.directorAgent = new ResearchDirectorAgent({
        model: config?.model,
        provider: config?.provider
      });

      // Define tasks for the synthesis team
      const evaluateResearchTask = new Task({
      ...
      const identifyInsightsTask = new Task({
      ...
      const createSummaryTask = new Task({
      ...
      const identifyGapsTask = new Task({
      ...
      const formatReportTask = new Task({
      ...
      logger.info('SynthesisTeam initialized successfully');
    } catch (error) {
      logger.error(`Failed to initialize SynthesisTeam: ${error}`);
      throw error;
    }
  }
  ...
  public async start(inputs: {
    ...
    try {
      logger.info(`Starting synthesis for topic: "${inputs.topic}"`);
      ...
      logger.error(`Synthesis process encountered an error: ${error}`);
      throw error;
    }
  }
